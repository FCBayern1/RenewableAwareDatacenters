...
public class simulation_test_firstfit {
    private static final Logger logger = LoggerFactory.getLogger(simulation_test_firstfit.class);
    private final inferenceConfig config;

    public simulation_test_firstfit(inferenceConfig config) {
        this.config = config;
    }

    public static void main(String[] args) {
        inferenceConfig config = new inferenceConfig();
        new simulation_test_firstfit(config).run();
    }

    public void run() {
        CloudSimPlus simulation = new CloudSimPlus(0.001);
        GlobalBrokerFirstFit globalBroker = new GlobalBrokerFirstFit(simulation);
